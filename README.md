...
Tested end to end on PostgreSQL 16. Everything used requires **PostgreSQL 14
or newer** (`DETACH PARTITION ... CONCURRENTLY` landed in 14); the compose file
pins 18.
...
# or, for a live walkthrough, one at a time:
psql -f sql/01_schemas_and_table.sql
```
...
a full-scan-under-ACCESS-EXCLUSIVE operation, and it **blocks
`DETACH CONCURRENTLY` entirely** — the one command this whole archiving
strategy depends on. Pre-create partitions and alert on the runway instead.
A failed insert at 3am is better than 400 million rows in `_default`.

**4. `DETACH ... CONCURRENTLY` cannot run inside a transaction — including
inside a function or procedure.** That is why the toolkit *generates* the DDL
and `\gexec` (or `archive.sh`) executes it. It also means pg_cron cannot run
the archive job as written.
...
(`ops.restore_plan`, in `08_...`). Expect `ATTACH` to rebuild the indexes you
...
- **A killed `DETACH CONCURRENTLY` leaves a half-detached partition.**
  `pg_inherits.inhdetachpending` is the tell; the fix is
  `ALTER TABLE ... DETACH PARTITION ... FINALIZE`. Monitor for it — the query
  is in `09_...` and should always return zero rows.
...
- **`CREATE INDEX CONCURRENTLY` is not supported on a partitioned table.**
  Use the three-step pattern: invalid index `ON ONLY` the parent →
  `CREATE INDEX CONCURRENTLY` per partition → `ALTER INDEX ... ATTACH
  PARTITION`. Runnable in `09_...`.
...
- **pg_partman.** It does everything in `02_` and `05_` and is maintained by
  other people. Use it in production. The value of these scripts is seeing
